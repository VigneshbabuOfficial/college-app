package com.college.faculty.utils;

public class Constants {

	public static final String ASC = "ASC";

	public static final String DESC = "DESC";
	public static final String DEFAULT_LIMIT_SIZE = "10";
	public static final String DEFAULT_PAGE_SIZE = "1";

	public static final String FACULTY_INPUT = "FACULTY_INPUT";
	public static final String FACULTY_ID = "FACULTY_ID";
	public static final String FIELDS = "fields";
	public static final String FILE_NAME = "file_name";
	public static final String FILTER = "filter";

	public static final String EMPTY_STR = "";
	public static final String EQUAL = "=";
	public static final String EXCEPTION = "exception";

	public static final String GREATER_THAN = ">";
	public static final String GRETER_THAN_OR_EQUAL_TO = ">=";

	public static final String ID = "id";
	public static final String ILIKE = "ILIKE";
	public static final String IMAGE = "image";
	public static final String IN = "IN";

	public static final String LESS_THAN = "<";
	public static final String LESS_THAN_OR_EQUAL_TO = "<=";
	public static final String LOCALDATETIME_FORMAT = "yyyy-MM-dd'T'HH:mm:ss'Z'";
	public static final String LIMIT = "limit";

	public static final String NAME = "NAME";
	public static final String NOT_EQUAL = "!=";
	public static final String NOT_ILIKE = "NOT ILIKE";
	public static final String NOT_IN = "NOT IN";

	public static final String OFFSET = "offset";
	public static final String OR = "OR";
	public static final String OR_FILTER = "ORfilter";

	public static final String PAGE = "page";

	public static final String RESPONSE_DTO = "responseDTO";

	public static final String SORT = "sort";
	public static final String STUDENT_INPUT = "STUDENT_INPUT";
	public static final String STUDENT_ID = "studentId";

	public static final String YYYY_MM_DD = "yyyy-MM-dd";

	private Constants() {
	}
}
